...
class PointMass(base.Task):
  ...
  def before_step(self, action, physics):
    physics.named.data.xfrc_applied['pointmass', :2] = 0
    physics.named.data.qfrc_applied[:2] = 0

    action = action * 0.075
    goal_position = physics.named.data.geom_xpos['pointmass', :2] + action
    dist = goal_position - physics.named.data.geom_xpos['pointmass', :2]

    loop = 0
    while np.linalg.norm(dist) > 0.005:
        loop += 1
        if loop > 100:
            break
        physics.named.data.xfrc_applied['pointmass', :2] = dist * 20
        physics.step()
