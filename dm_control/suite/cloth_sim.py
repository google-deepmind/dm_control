...
class Stack(base.Task):
  ...
  def action_spec(self, physics):
    ...
    return specs.BoundedArraySpec(
      shape=(5,), dtype=np.float, minimum=[-1.0,-1.0,-1.0,0,0] , maximum=[1.0,1.0,1.0,64,64])
  ...
  def before_step(self, action, physics):
    ...
    # hyperparameter epsilon=2 and select the point
    epsilon=10
    possible_index=[]
    possible_z=[]
    for i in range(86):
        if abs(cam_pos_xy[i][0]-location[0])<epsilon and abs(cam_pos_xy[i][1]-location[1])<epsilon and i>4:
