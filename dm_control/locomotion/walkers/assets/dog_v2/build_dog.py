...
def exclude_contacts(model):
    ...
    # manual exclusions
    model.contact.add('exclude', name='C_1:jaw', body1=model.find('body', 'C_1'), body2=model.find('body', 'jaw'))
    model.contact.add(
        'exclude', name='torso:lower_arm_L', body1=model.find('body', 'torso'), body2='lower_arm_L')
    model.contact.add(
        'exclude', name='torso:lower_arm_R', body1=model.find('body', 'torso'), body2='lower_arm_R')
    model.contact.add(
...
def main(argv):
    ...
    pelvic_bones, lumbar_joints = build_torso.create_torso(model,
        bones, bone_position, lumbar_dofs_per_veterbra, side_sign,
    ...
    cervical_joints = build_neck.create_neck(model, bone_position,
        cervical_dofs_per_veterbra, bones, side_sign, bone_size,
        parent=model.find('body', 'torso'))

    print('Back legs.')
    nails, sole_sites = build_back_legs.create_back_legs(model,
        primary_axis, bone_position, bones, side_sign, bone_size, pelvic_bones,
        parent=model.find('body', 'pelvis'))

    print('Shoulders, front legs.')
    palm_sites = build_front_legs.create_front_legs(nails,
        model, primary_axis, bones, side_sign, parent=model.find('body', 'torso'))

    print('Tail.')
    caudal_joints = build_tail.create_tail(caudal_dofs_per_veterbra,
        bone_size, model, bones, bone_position, parent=model.find('body', 'pelvis'))
    ...
    actuated_joints = add_torque_actuators.add_motors(physics, model,
        lumbar_joints, cervical_joints, caudal_joints)
    ...
    physics = mjcf.Physics.from_mjcf_model(model)
    # sensors
    model.sensor.add('accelerometer', name='accelerometer', site=model.find('site', 'head'))
    model.sensor.add('velocimeter', name='velocimeter', site=model.find('site', 'head'))
    model.sensor.add('gyro', name='gyro', site=model.find('site', 'head'))
    model.sensor.add('subtreelinvel', name='torso_linvel', body=model.find('body', 'torso'))
    model.sensor.add('subtreeangmom', name='torso_angmom', body=model.find('body', 'torso'))
