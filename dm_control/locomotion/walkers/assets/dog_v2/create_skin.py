...
def create(model, mesh_file, mesh_name, asset_dir, tex_coords=True, transform=True):
    ...
    if tex_coords:
        ...
        n_tex_coord = 0
    f.write(struct.pack("4i", nvert, n_tex_coord, skin.face.size // 3, len(skin.bone)))
    f.write(struct.pack(str(skin.vertex.size) + "f", *skin.vertex))

    if n_tex_coord:
        assert physics.model.mesh_texcoord.shape[0] == physics.bind(mesh_file).vertnum
        f.write(
    ...
    for bone in skin.bone:
        ...
        f.write(struct.pack(str(name_length) + "c", *[s.encode() for s in bone.body]))
