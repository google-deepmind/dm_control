...
def create_back_legs(model, primary_axis, bone_position, bones, side_sign,
  ...
  for side in ['_L', '_R']:
    pelvis.add(
      'site', name='hip' + side,
      size=[0.011], pos=hip_pos * side_sign[side])
  ...
  for side in ['_L', '_R']:
    ...
    for dof in ['_supinate', '_abduct', '_extend']:
      ...
      leg.add('joint', name='hip' + side + dof,
              dclass='hip' + dof, axis=axis)
  ...
  for side in ['_L', '_R']:
    ...
    leg.add('site', type='cylinder', name='ankle' + side,
            size=[0.003, .013], zaxis=(0, 1, 0),
